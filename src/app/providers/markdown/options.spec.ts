...
describe('Production Markdown renderer ownership', () => {
  it('[social-markdown-link-receiver] renders inline links through the active parser and preserves local navigation', async () => {
  ...
  it('[social-markdown-external-policy] keeps external tab and rel policy without losing body content', async () => {
    const parser = new Marked({renderer: markedOptionsFactory().renderer});
  ...
  it('[social-markdown-image-receiver] renders image alt tokens with CMS-relative URL policy', async () => {
    const parser = new Marked({renderer: markedOptionsFactory().renderer});
