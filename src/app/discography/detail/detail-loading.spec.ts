...
describe('Detail content pending owns HTTP, not artwork events', () => {
  ...
  for (const artwork of [null, {url: '/synthetic-cover.png', alternativeText: 'Synthetic cover'}]) {
    it(`settles successful content with ${artwork ? 'failed artwork' : 'absent artwork'}`, () => {
  ...
  it('settles failed HTTP before its fallback navigation completes', async () => {
  ...
  it('cancels outgoing HTTP without clearing a newer load', () => {
    const fixture = TestBed.createComponent(DetailComponent);fixture.detectChanges();
    const http = TestBed.inject(HttpTestingController);
