...
describe('Given the blog index receives article and count responses', () => {
  ...
  it('When creates the page after its initial requests settle', () => {
  ...
  it('When [blog-http-error-loading] handles a failed page without an unhandled error and releases loading', async () => {
    http.expectOne(`${environment.cmsUrl}/blogs/count`).flush(0);
  ...
  it('When handles a count failure separately and retries without hiding successful articles', async () => {
    http.expectOne(pageUrl(1)).flush([blog(1)]);
  ...
  it('When [blog-concurrent-page-order] cancels the older page and only the current request settles loading', () => {
    http.expectOne(`${environment.cmsUrl}/blogs/count`).flush(12);
  ...
  it('When does not display empty success while pending and cancels both requests on destroy', () => {
    expect(screen.queryByText('まだ記事がありません')).toBeNull();
