...
describe('Given social list and tag navigation receive route and query state', () => {
  ...
  it('When [social-tag-loading] distinguishes pending tag results from an empty success', async () => {
  ...
  it('When [social-tag-reuse] replaces a reused tag route and cancels stale data/title/loading ownership', async () => {
    const harness = await RouterTestingHarness.create('/blog/tag/Music');
  ...
  it('When [social-tag-retry] owns failure, retry and destruction without unhandled HTTP errors', async () => {
    const harness = await RouterTestingHarness.create('/blog/tag/Music');
  ...
  it('When [social-tag-page-url] loads a literal tag and its page from URL; query-only history does not refetch', async () => {
    const tag = 'C++ / A&B?#日本';
  ...
  it('When [social-list-page-url] opens page two cold and restores query transitions instead of resetting to page one', async () => {
  ...
  it('When [social-list-invalid-page] normalizes invalid page with replacement while preserving unrelated query', async () => {
